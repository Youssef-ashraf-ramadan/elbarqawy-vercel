...
import { getSafeDetails, updateSafe, clearError, clearSuccess, clearSafeDetails } from '../../../../redux/Slices/authSlice';
...
const EditSafe = () => {
  ...
  const { id } = useParams();
  const { safeDetails, isLoading, error, success } = useSelector((state) => state.auth);
  ...
  useEffect(() => {
    if (safeDetails) {
      const details = safeDetails.data || safeDetails;
      setFormData({
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(updateSafe({ id, data: submitData }));
  };

  if (isLoading && !safeDetails) {
    return (
      <div style={{ 
        padding: '30px', 
  ...
  return (
    <div style={{ 
      ...
      <div style={{ marginBottom: '30px' }}>
        ...
        <h1 style={{ fontSize: '24px', fontWeight: 'bold', margin: 0 }}>تعديل الخزنة</h1>
      </div>
