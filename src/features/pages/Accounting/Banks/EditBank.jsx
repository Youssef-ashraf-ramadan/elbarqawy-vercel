...
import { getBankDetails, updateBank, clearError, clearSuccess, clearBankDetails } from '../../../../redux/Slices/authSlice';
...
const EditBank = () => {
  ...
  const { id } = useParams();
  const { bankDetails, isLoading, error, success } = useSelector((state) => state.auth);
  ...
  useEffect(() => {
    if (bankDetails) {
      const details = bankDetails.data || bankDetails;
      setFormData({
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(updateBank({ id, data: submitData }));
  };
  ...
  return (
    <div style={{ 
      ...
      <div style={{ marginBottom: '30px' }}>
        ...
        <h1 style={{ fontSize: '24px', fontWeight: 'bold', margin: 0 }}>تعديل البنك</h1>
      </div>
