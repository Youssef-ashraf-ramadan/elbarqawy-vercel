...
const PaymentVoucherDetails = () => {
  ...
  const { paymentVoucherDetails, isLoading, error } = useSelector((state) => state.auth);
  ...
  if (isLoading || !paymentVoucherDetails) {
    return (
  ...
  const voucher = paymentVoucherDetails;
    ...
    <div style={{ padding: '30px', backgroundColor: '#121828', minHeight: 'calc(100vh - 80px)', color: 'white' }}>
      ...
      <div style={{ backgroundColor: '#202938', border: '1px solid #333', borderRadius: '12px', overflow: 'hidden' }}>
        <div style={{ padding: '16px', borderBottom: '1px solid #333', backgroundColor: '#1a1f2e' }}>
          <strong>تفاصيل سند الصرف #{voucher.id}</strong>
        </div>
                ...
                <thead>
                  <tr style={{ backgroundColor: '#1a1f2e' }}>
                    {[
                      ...
                      { label: 'مركز التكلفة', width: '200px' },
                ...
                <tbody>
                  {details.map((detail, index) => (
                    ...
                    >
                      ...
                      <td style={{ padding: '18px 16px', textAlign: 'center', color: 'white', fontSize: '14px' }}>
                        {detail.cost_center
                          ? detail.cost_center.code
                            ? `${detail.cost_center.code} - ${detail.cost_center.name}`
                            : detail.cost_center.name
                          : '-'}
                      </td>
                      <td style={{ padding: '18px 16px', textAlign: 'center', color: 'white', fontSize: '14px' }}>
                        {detail.description || '-'}
