...
const PaymentVouchers = () => {
  ...
  useEffect(() => {
    dispatch(getPaymentVouchers({ page: currentPage, per_page: 10 }));
  ...
  const confirmDelete = () => {
    ...
    dispatch(deletePaymentVoucher(voucherToDelete.id));
    closeDeleteModal();
  };
  ...
  const renderPagination = () => {
    if (!paymentVouchersPagination || (paymentVouchers || []).length === 0) {
      return null;
    }

    const current = paymentVouchersPagination.current_page || 1;
    const last = paymentVouchersPagination.last_page || 1;
  ...
  if (isLoading && (!paymentVouchers || paymentVouchers.length === 0)) {
    return (
      <div
        style={{
          padding: '30px',
  ...
  return (
    <div style={{ padding: '30px', backgroundColor: '#121828', minHeight: 'calc(100vh - 80px)', color: 'white' }}>
      <div style={{ marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h1 style={{ fontSize: '20px', margin: 0 }}>سندات الصرف</h1>
        <button
          onClick={() => navigate('/payment-vouchers/add')}
          style={{
            ...
            <tbody>
              {(paymentVouchers || []).length === 0 ? (
                ...
                (paymentVouchers || []).map((voucher, idx) => {
                  ...
                  const paginationMeta = paymentVouchersPagination || {};
                    ...
                    >
                      ...
                      <td style={{ padding: '18px 16px', textAlign: 'center' }}>
                        ...
                        >
                          <button
                            title="عرض"
                            onClick={() => navigate(`/payment-vouchers/view/${voucher.id}`)}
                            style={{
                              backgroundColor: '#666',
                              color: 'white',
                              border: 'none',
