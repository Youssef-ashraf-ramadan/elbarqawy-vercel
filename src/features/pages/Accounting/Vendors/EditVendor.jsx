...
import { getVendorDetails, updateVendor, clearError, clearSuccess, clearVendorDetails } from '../../../../redux/Slices/authSlice';
...
const EditVendor = () => {
  ...
  const { id } = useParams();
  const { vendorDetails, isLoading, error, success } = useSelector((state) => state.auth);
  ...
  useEffect(() => {
    if (vendorDetails) {
      const details = vendorDetails.data || vendorDetails;
      setFormData({
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    dispatch(updateVendor({ id, data: formData }));
  };
  ...
  return (
    <div style={{ 
      ...
      <h1 style={{ fontSize: '24px', fontWeight: 'bold', marginBottom: '30px' }}>تعديل المورد</h1>
